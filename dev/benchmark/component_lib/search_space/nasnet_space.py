...
class DropPath(tf.keras.layers.Layer):
    ...
    def call(self, x, training=None):
        ...
        return tf_utils.smart_cond(training,
                                   lambda: apply_drop_path(x, keep_prob),
                                   lambda: x)
...
def drop_path(cell_ratio):
    ...
    return htfe.TensorflowEagerModule('DropPath', {
    ...
    def generate(filters, cell_ratio):
        return cell(
            ...
            lambda in_id, node_id, op_num: intermediate_node_fn(
                reduction, in_id, node_id, op_num, filters, cell_ratio, cell_ops
            ), lambda: add(2), combine_unused, num_nodes, connection_hparams)
...
class SSF_NasnetA(SearchSpaceFactory):

    def __init__(self, exp_config):
        SearchSpaceFactory.__init__(
            self, lambda: generate_search_space(5, 18, 2, 36, 3.0), exp_config)
