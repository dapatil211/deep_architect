...
def concat(num_ins):
    ...
    def compile_fn(di, dh):
        ...
        def forward_fn(di, is_training=True):
            return {
                'Out':
                    tf.concat(values=[di[input_name] for input_name in di],
                              axis=3)
            }
    ...
    def generate(filters):
        return cell(
            ...
            lambda num_inputs, node_id, channels: intermediate_node_fn(
                num_inputs, node_id, channels, cell_ops), concat, h_connections,
            5, filters)
...
class Nasbench_SSF(SearchSpaceFactory):

    def __init__(self, exp_config):
        SearchSpaceFactory.__init__(self,
                                    lambda: generate_search_space(3, 3, 5, 128),
                                    exp_config)
